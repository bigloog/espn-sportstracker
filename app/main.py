...
def fetch_team_data(league_slug: str, team_id: int):
...
def get_cached_team_data(league_slug: str, team_slug: str, team_id: int):
    ...
    data = fetch_team_data(league_slug, team_id)
    cache[key] = (now, data)
    return data
...
def api_team(team_slug: str, league_slug: str):
    # Find team in config by slug and league
    for team_key, team_info in config.get("teams", {}).items():
        if team_info["espn_slug"] == team_slug and team_info["league"] == league_slug:
            team_id = team_info.get("espn_id")
            if not team_id:
                raise HTTPException(status_code=404, detail="Team ESPN ID not found in config")
            return get_cached_team_data(league_slug, team_slug, team_id)
...
def api_team_no_league(team_slug: str):
    # Find team in config by slug (any league)
    for team_key, team_info in config.get("teams", {}).items():
        if team_info["espn_slug"] == team_slug:
            team_id = team_info.get("espn_id")
